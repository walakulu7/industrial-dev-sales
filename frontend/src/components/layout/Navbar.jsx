...
import { FaUserCircle, FaCaretDown, FaUser, FaKey, FaSignOutAlt, FaBars } from "react-icons/fa";
...
const Navbar = ({ toggleSidebar }) => { 
    ...
    return (
        <div className="bg-white h-16 shadow-sm flex justify-between items-center px-4 md:px-6 relative z-10 sticky top-0">
            
            {/* LEFT: Toggle Button Only (Text Removed) */}
            ...
            <div className="relative" ref={dropdownRef}>
                <button 
                    ...
                    className="flex items-center gap-2 hover:bg-gray-50 p-2 rounded transition focus:outline-none"
                >
                ...
                {dropdownOpen && (
                    <div className="absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-xl border border-gray-100 py-2 animate-fade-in-up">
                        ...
                        <Link to="/profile" onClick={() => setDropdownOpen(false)} className="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-indigo-50 hover:text-indigo-600">
                            <FaUser className="mr-3" /> Profile
                        </Link>
                        <button onClick={() => { setDropdownOpen(false); setShowPwdModal(true); }} className="w-full flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-indigo-50 hover:text-indigo-600">
                ...
                <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50 p-4">
                    <div className="bg-white p-6 rounded shadow-xl w-full max-w-sm">
                        ...
                        <form onSubmit={handleChangePassword}>
                            <input type="password" placeholder="Current Password" className="w-full border p-2 rounded mb-3" required onChange={e => setPasswords({...passwords, current: e.target.value})} />
                            <input type="password" placeholder="New Password" className="w-full border p-2 rounded mb-4" required onChange={e => setPasswords({...passwords, new: e.target.value})} />
