...
exports.recordProduction = async (req, res) => {
    const { 
        center_id, date, 
        input_product_id, input_warehouse_id, input_qty,
        output_product_id, output_warehouse_id, output_qty,
        user_id 
    } = req.body;
    ...
    try {
        ...
        if (stock.length === 0 || parseFloat(stock[0].quantity_on_hand) < parseFloat(input_qty)) {
        ...
        await connection.query(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [date, center_id, input_product_id, input_qty, output_product_id, output_qty, user_id]
        );
